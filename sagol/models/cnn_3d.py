...
class ConvNet(nn.Module):
    ...
    def __init__(self, out_dim, kernel_size, filters, max_pool_size, max_pool_every, batch_norm_every, hidden_dims,
        ...
        if not ((hidden_dims_regressor is None) ^ (hidden_dims_classifier is None)):
            raise RuntimeError('Network must be exactly one of the two: a regressor or a classifier! Exactly one of the '
        ...
        with torch.no_grad():
            if batch_size > 0:
                ...
                for i in range(0, len(X), batch_size):
                    ...
                    ten = torch.cat(
                        (ten, self(X[i:finish_idx]) if self.is_reg else torch.Tensor([0 if y < 0.5 else 1 for y in self(X)])))
                return ten
            else:
...
def train_cnn_3d(X_train, y_train, **kwargs):
    ...
    optimizer = torch.optim.Adam(params=cnn.parameters(), lr=lr, betas=(beta_1, beta_2), eps=eps, weight_decay=weight_decay, amsgrad=amsgrad)
    train_loss, kwargs['epochs_completed'] = cnn.fit(X=X_train, y=y_train, optimizer=optimizer, loss_func=loss_func, epochs=epochs,
                                                     batch_size=batch_size, early_stopping=early_stopping, print_loss_every=-1)
    del kwargs['in_size']
